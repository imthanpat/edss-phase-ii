  ...
  <v-navigation-drawer floating color="#f8f8f8" absolute height="100%">
    ...
    <v-list dense nav>
      ...
      <v-list-group prepend-icon="mdi-archive" value="Project">
        ...
        <v-list-item v-for="([title, val, link], i) in projects" :key="i" :value="val" :title="title" :to="link" link>
        </v-list-item>
      </v-list-group>
      ...
      <v-list-group prepend-icon="mdi-chip" value="Device Management">
        ...
        <v-list-item v-for="([title, val, link], i) in deviceMgmt" :key="i" :value="val" :title="title" :to="link" link>
        </v-list-item>
      </v-list-group>

      <v-list-item title="Map Layout" value="Map Layout" prepend-icon="mdi-map"></v-list-item>

      <v-list-group prepend-icon="mdi-bag-checked" value="Device Inventory">
        <template v-slot:activator="{ props }">
          <v-list-item v-bind="props" title="Device Inventory"></v-list-item>
        </template>

        <v-list-item v-for="([title, val, link], i) in inventory" :key="i" :value="val" :title="title" :to="link" link>
        </v-list-item>
      </v-list-group>

      <v-list-group prepend-icon="mdi-trash-can" value="Device Unlist">
        <template v-slot:activator="{ props }">
          <v-list-item v-bind="props" title="Device Unlist"></v-list-item>
        </template>
        <v-list-item v-for="([title, val, link], i) in unlist" :key="i" :value="val" :title="title" :to="link" link>
        </v-list-item>
      </v-list-group>

      <v-list-group prepend-icon="mdi-recycle-variant" value="Device Reuse">
        <template v-slot:activator="{ props }">
          <v-list-item v-bind="props" title="Device Reuse"></v-list-item>
        </template>

        <v-list-item v-for="([title, val, link], i) in reuse" :key="i" :value="val" :title="title" :to="link" link>
        </v-list-item>
      </v-list-group>
      ...
      <v-list-group prepend-icon="mdi-cog-outline" value="FM Config">
        ...
        <v-list-item v-for="([title, val, link], i) in fmconf" :key="i" :value="val" :title="title" :to="link" link>
        </v-list-item>
      </v-list-group>
      ...
      <v-list-group prepend-icon="mdi-file-document" value="Alarm Logs">
        ...
        <v-list-item v-for="([title, val, link], i) in alarmlogs" :key="i" :value="val" :title="title" :to="link" link>
        </v-list-item>
      </v-list-group>

      <v-list-group prepend-icon="mdi-account-box-multiple" value="User Management">
        <template v-slot:activator="{ props }">
          <v-list-item v-bind="props" title="User Management"></v-list-item>
        </template>
        <v-list-item v-for="([title, val], i) in usermgmt" :key="i" :value="title" :title="title">
        </v-list-item>
      </v-list-group>
...
export default {
  ...
  data() {
    return {
      ...
      deviceMgmt: [
        ['Dev-Region', 'Dev-Region', '/deviceoverview'],
        ['Add / Import', 'addImportDevice', '/device/add'],
        ['Overview', 'deviceOverview', '/device/overview'],
        ['A-Box', 'deviceAbox', '/device/abox'],
        ['R-Box', 'deviceRbox', '/device/rbox'],
        ['Device-Status', 'deviceStatus', '/device/status'],
        ['Find-Device', 'findDevice', '/device/find']
      ],
      inventory: [
      ...
      fmconf: [
        ...
        ['A-Box : Rule', 'fmconf-a-box', ''],
        ['W-Box : Rule', 'fmconf-w-box', ''],
        ['TH-Box : Rule', 'fmconf-th-box', ''],
        ['VA-Box : Rule', 'fmconf-va-box', ''],
        ['R-Box : Rule', 'fmconf-r-box', '']
      ],
      alarmlogs: [
  ...
  watch: {
    ...
    listEnable: function(val) {
      ...
      tmpDeviceMgmt.unshift(['Dev-Region', 'Dev-Region', '/deviceoverview'],['Add / Import', 'addImportDevice', '/device/add'], ['Overview', 'deviceOverview', '/device/overview']);
      tmpDeviceMgmt.push(['Device-Status', 'deviceStatus', '/device/status'],['Find-Device', 'findDevice', '/device/find']);
