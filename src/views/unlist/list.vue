<template>
  ...
  >
    <template #item-status="item">
      <v-chip
        v-if="item.last_update != 'Invalid Date'"
        size="small"
        class="ma-2"
        color="success"
        label
...
export default {
  ...
  methods: {
    submitSearch() {
      ...
      if (this.searchTxt.length > 0) {
        ...
        } else if (this.selected.val == "addr") {
          _filter = _filter.filter(
            (x) =>
              x.addr != null &&
              x.addr.toLowerCase().includes(this.searchTxt.toLowerCase())
          );
        } else {
          _filter = _filter.filter(
            (x) =>
    ...
    editInfo(item) {
      ...
      this.editModel.info.project = "UNLIST";
  ...
  mounted() {
    ProjectApi.PhpProjectList()
        .then((response) => {
          this.unlistId = response.data.filter(
            (x) =>
              x.name == "UNLIST"
          )[0].id;
          this.loadInfo(this.unlistId);
