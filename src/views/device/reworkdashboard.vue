...
import rboxJson from "../../assets/ii-r-box.json";
import ProjectApi from "../../services/ProjectApi";
//import rboxJson from "../../assets/old-r-box.json";
...
export default {
  ...
  watch: {
    ...
    projectId(newvalue, oldvalue) {
      this.rboxInProject(newvalue);
    },
  },
  methods: {
    SetAll(){
      this.items.forEach((_item) => {
        this.SetDashboard(_item);
      });
    },
    rboxInProject(currentProjectId){
      DeviceMgmtApi.DeviceList(currentProjectId, "R-Box")
        .then((response) => {
    ...
    SetDashboard(item) {
      let dashboardId = (item.dashboard_url.split("?publicId=")[0]).split("/dashboards/").slice(-1)[0];
      let imei = item.imei;
      let deviceId = item.device_id;
      let pj_id = this.projectId;

      let _idRef = {};
      _idRef.entityType = "DASHBOARD";
      _idRef.id = dashboardId.toString();

      let _payload = rboxJson;
      _payload['title'] = imei.toString();
      _payload['name'] = imei.toString();
      _payload['configuration']['entityAliases']['b33b87cb-7dfb-aad0-e2b1-bd061c3ff269']['filter']['singleEntity'].id = deviceId.toString();
      _payload.id = _idRef;

      DeviceMgmtApi.SetDashboard(_payload)
        .then(res => {
          let xpayload = pj_id + "/device/" + deviceId;
            ProjectApi.MoveDeviceToNewProject(xpayload).then((xresponse) => {
              DeviceMgmtApi.FixDashboardIssue(dashboardId, pj_id).then();
              console.log(imei + " [Complete]");
            });
        }).catch(err => {
            console.log(err);
        });
  ...
  mounted() {
    this.rboxInProject(this.projectId);
  },
  data() {
    return {
      nodata: false,
