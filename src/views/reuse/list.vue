<template>
      ...
      <!-- <v-row no-gutters>
        ...
        <v-col cols="6" class="pa-2">
          <v-radio-group v-model="selDateSearchType" class="text-left">
            <v-radio value="datefirstinstall">
              <template v-slot:label> Date First Install </template>
            </v-radio>
            <v-radio value="dateinstock">
              <template v-slot:label> Date In Stock </template>
            </v-radio>
          </v-radio-group>
        </v-col>
      </v-row>
  ...
  >
    <template #item-status_reuse="item">
      <v-chip class="ma-2" color="indigo" size="small" label> Reuse </v-chip>
    </template>
    <template #item-maintenance="item">
      <v-chip
        v-if="item.maintenance != 'true'"
        size="small"
        class="ma-2"
        color="teal"
        text-color="white"
...
export default {
  ...
  data() {
    return {
      selDateSearchType: "datefirstinstall",
      tab: null,
      date: null,
      searchTxt: "",
      selSearch: [
  ...
  methods: {
    submitSearch() {
      ...
      //   if (this.selDateSearchType == "datefirstinstall") {
      //     _filter = _filter.filter(
      //       (x) => x.storeDateFirstInstall >= s_date && x.storeDateFirstInstall <= e_date
      //     );
      //   } else {
      //     _filter = _filter.filter(
      //       (x) => x.storeDateInStock >= s_date && x.storeDateInStock <= e_date
      //     );
      //   }
      // }
    ...
    editInfo(item) {
      ...
      this.editModel.info.project = "REUSE";
    ...
    submitMoveBtnClick() {
      ...
      ProjectApi.MoveDeviceToNewProject(payload)
        .then((response) => {
          // Fix Dashboard Issue
          DeviceMgmtApi.FixDashboardIssue(this.sel_dashboard, this.sel_project_dest).then();
    ...
    moveBtnClick(item) {
      this.sel_dashboard = (item.dashboard_url.split("?publicId=")[0]).split("/dashboards/").slice(-1)[0];
      this.sel_device_from = item.device_id;
    ...
    loadInfo() {
      ...
      this.pageType = types;
      ReuseApi.ReuseInfo(this.pageType, this.reuseId)
        .then((response) => {
  ...
  mounted() {
    ProjectApi.PhpProjectList()
        .then((response) => {
          let _check = response.data.filter(
            (x) =>
              x.name == "REUSE"
          )[0];
          this.reuseId = _check.id;
          this.loadInfo();
        })
        .catch((err) => {
          console.log(err);
