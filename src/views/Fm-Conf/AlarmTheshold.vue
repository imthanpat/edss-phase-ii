<template>
    <div class="d-flex flex-column pr-5 pl-5">
        <v-card class="mt-5">
            <v-card-item>
                <div>
                    <div class="text-overline">
                        FALSE MANAGEMENT CONFIG
                    </div>
            ...
            <v-row no-gutters class="align-center fill-height ">
                ...
                <v-col cols="6" v-if="listEnable.abox">
                    <v-banner lines="two" color="deep-purple-accent-4">
                        <template v-slot:prepend>
                            <v-chip class="ml-10" variant="outlined">
                                A-Box
                            </v-chip>
                        </template>
                        <template v-slot:text>
                            <v-select v-model="aboxThesholdCurrent" :items="thesholds" label="alarm theshold (* min)"
                                persistent-hint return-object single-line style="max-width: 400px;"></v-select>
                        </template>
                ...
                <v-col cols="6" v-if="listEnable.wbox">
                    <v-banner lines="two" color="deep-purple-accent-4" m>
                        ...
                        <template v-slot:text>
                            <v-select v-model="wboxThesholdCurrent" :items="thesholds" label="alarm theshold (* min)"
                                persistent-hint return-object single-line style="max-width: 400px;"></v-select>
                        </template>
                    </v-banner>
                </v-col>
                <v-col cols="6" v-if="listEnable.vabox">
                    <v-banner lines="two" color="deep-purple-accent-4">
                        ...
                        <template v-slot:text>
                            <v-select v-model="vaboxThesholdCurrent" :items="thesholds" label="alarm theshold (* min)"
                                persistent-hint return-object single-line style="max-width: 400px;"></v-select>
                        </template>
                    </v-banner>
                </v-col>
                <v-col cols="6" v-if="listEnable.rbox">
                    <v-banner lines="two" color="deep-purple-accent-4">
                        ...
                        <template v-slot:text>
                            <v-select v-model="rboxThesholdCurrent" :items="thesholds" label="alarm theshold (* min)"
                                persistent-hint return-object single-line style="max-width: 400px;"></v-select>
                        </template>
                    </v-banner>
                </v-col>
                <v-col cols="6" v-if="listEnable.thbox">
                    <v-banner lines="two" color="deep-purple-accent-4">
                        ...
                        <template v-slot:text>
                            <v-select v-model="thboxThesholdCurrent" :items="thesholds" label="alarm theshold (* min)"
                                persistent-hint return-object single-line style="max-width: 400px;"></v-select>
                        </template>
                    </v-banner>
                </v-col>
        ...
        <v-alert class="mt-5" variant="outlined" type="warning" prominent border="top">
  ...
  export default {
      ...
      methods: {
        ...
        submitEvent(){
            ...
            preConfirm: (pass) => {
                return LoginApi.AuthenEDSS({ username: localStorage.getItem('username'), password: pass })
                .then(response => {
            ...
            allowOutsideClick: () => !this.$swal.isLoading()
                ...
                let sel_pj_id = this.projects.filter((x) => x.name.toString()
                .toLowerCase()
        ...
        searchPj(){
            ...
            FmconfApi.GetListScope(sel_pj_id)
              .then(response => {
                this.listEnable.abox = (response.find(o => o.key === 'A-Box').value == true) || (response.find(o => o.key === 'A-Box').value == "true") ? true : false;
                this.listEnable.wbox = (response.find(o => o.key === 'W-Box').value == true) || (response.find(o => o.key === 'W-Box').value == "true") ? true : false;
                this.listEnable.vabox = (response.find(o => o.key === 'VA-Box').value == true) || (response.find(o => o.key === 'VA-Box').value == "true") ? true : false;
                this.listEnable.rbox = (response.find(o => o.key === 'R-Box').value == true) || (response.find(o => o.key === 'R-Box').value == "true") ? true : false;
                this.listEnable.thbox = (response.find(o => o.key === 'TH-Box').value == true) || (response.find(o => o.key === 'TH-Box').value == "true") ? true : false;
                ...
                if(this.listEnable.vabox){ this.rboxThesholdCurrent = response.find(o => o.key === 'threshold_rbox').value;  }
                if(this.listEnable.vabox){ this.thboxThesholdCurrent = response.find(o => o.key === 'threshold_thbox').value;  }
      ...
      watch: {
            aboxThesholdCurrent(val) {
                ...
                (this.wboxThesholdCurrent == this.wboxThesholdTmp) &&
                    (this.vaboxThesholdCurrent == this.vaboxThesholdTmp) &&
                        (this.rboxThesholdCurrent == this.rboxThesholdTmp) &&
                            (this.thboxThesholdCurrent == this.thboxThesholdTmp)){ this.isSubmit = true; }
      ...
      async created() {
          ...
          FmconfApi.GetProjectList()
              .then(response => {
                  this.projects = response.data.filter(x => x.name != "INVENTORY" && x.name != "UNLIST");
                  ...
                  FmconfApi.GetListScope(sel_pj_id)
                    .then(res => {
                        this.listEnable.abox = (res.find(o => o.key === 'A-Box').value == true) || (res.find(o => o.key === 'A-Box').value == "true") ? true : false;
                        this.listEnable.wbox = (res.find(o => o.key === 'W-Box').value == true) || (res.find(o => o.key === 'W-Box').value == "true") ? true : false;
                        this.listEnable.vabox = (res.find(o => o.key === 'VA-Box').value == true) || (res.find(o => o.key === 'VA-Box').value == "true") ? true : false;
                        this.listEnable.rbox = (res.find(o => o.key === 'R-Box').value == true) || (res.find(o => o.key === 'R-Box').value == "true") ? true : false;
                        this.listEnable.thbox = (res.find(o => o.key === 'TH-Box').value == true) || (res.find(o => o.key === 'TH-Box').value == "true") ? true : false;

                        this.aboxThesholdCurrent = res.find(o => o.key === 'threshold_abox').value;  
                        this.wboxThesholdCurrent = res.find(o => o.key === 'threshold_wbox').value;  
                        this.vaboxThesholdCurrent = res.find(o => o.key === 'threshold_vabox').value;  
                        this.rboxThesholdCurrent = res.find(o => o.key === 'threshold_rbox').value;  
                        this.thboxThesholdCurrent = res.find(o => o.key === 'threshold_thbox').value;  
