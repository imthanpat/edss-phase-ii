<template>
  <v-app>
    ...
    <v-main>
      ...
      <v-container fluid style="height: 100%;-webkit-fill-available!">
        <v-app-bar
          id="search-bar"
...
export default {
  ...
  created() {
    ...
    ProjectApi.GetProjectList()
      .then((response) => {
        let _userProjects = localStorage.getItem("pjId").split(",");
        ...
        if (this.level != "superadmin") {
          this.projects = this.projects.filter((y) =>
            _userProjects.includes(y.id.id)
          );
        }
